...
for (date in valid_dates) {
  ...
  month_data <- subset(census_results5, agg_date == date, select = c("agg_date", "tractce10", "avg_daily_co", "total_population", "roofs_2012", "median_home_value", "conservation"))

  # Fit the model
  hv_r_c_co_lm <- lm(avg_daily_co ~ median_home_value + roofs_2012 + conservation, data = month_data)

  # Get the summary
  hv_r_c_co_sum <- summary(hv_r_c_co_lm)

  # Extract coefficients
  month_coefficients <- hv_r_c_co_sum$coefficients
...
# Export to Excel
write.xlsx(results_df, file = "Excel Results/hv_r_c_co.xlsx", sheetName = "Sheet1", row.names = FALSE)
