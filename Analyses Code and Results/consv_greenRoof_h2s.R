...
for (date in valid_dates) {
  ...
  month_data <- subset(census_results5, agg_date == date, select = c("agg_date", "tractce10", "avg_daily_h2s", "conservation", "roofs_2012"))
  
  # Fit the model
  consv_gr_h2s_lm <- lm(avg_daily_h2s ~ conservation + roofs_2012, data = month_data)
  
  # Get the summary
  consv_gr_h2s_sum <- summary(consv_gr_h2s_lm)
  
  # Extract coefficients
  month_coefficients <- consv_gr_h2s_sum$coefficients
...
# Export to xlsx
write.xlsx(results_df, file = "Excel Results/consv_greenRoof_h2s.xlsx", sheetName = "Sheet1", row.names = FALSE)
