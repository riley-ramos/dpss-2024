census_results6 <- census_results4 %>%
  filter(agg_date >= "2018-02-01", 
         agg_date <= "2019-02-01", 
          agg_date != "2018-08-01") 
#          agg_date != "2018-09-01")

valid_dates <- unique(census_results6$agg_date)
...
for (date in valid_dates) {
  ...
  month_data <- subset(census_results6, agg_date == date, select = c("agg_date", "tractce10", "avg_daily_h2s", "total_population", "roofs_2012", "median_home_value", "conservation")) 
  
  # Fit the model
  hv_r_c_h2s_lm <- lm(avg_daily_h2s ~ median_home_value + roofs_2012 + conservation, data = month_data)
  
  # Get the summary
  hv_r_c_h2s_sum <- summary(hv_r_c_h2s_lm)
  
  # Extract coefficients
  month_coefficients <- hv_r_c_h2s_sum$coefficients
...
# Export to Excel
write.xlsx(results_df, file = "Excel Results/hv_r_c_h2s.xlsx", sheetName = "Sheet1", row.names = FALSE)
...
#   scale_x_discrete(breaks = unique(results_df$agg_date))