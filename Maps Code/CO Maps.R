...
census_ap_3 <- census_ap_df %>%
  distinct(tractce10, tract_id, agg_date, avg_daily_co) %>%
  group_by(tractce10, tract_id, agg_date) %>%
  summarize(
    avg_monthly_co = mean(avg_daily_co, na.rm=T)
  )
view(census_ap_1)

# Remove geometry to turn into df
census_ap_3$geometry = NULL

# Join with tracts to get polygon geometries
census_ap_3 <- inner_join(census_ap_3, tracts, by="tract_id")

# Convert to sf object for mapping
census_ap_3 <- st_as_sf(census_ap_3)

# filter for valid dates
census_ap_3 <- census_ap_3 %>%
  filter(agg_date >= "2018-02-01", 
         agg_date <= "2019-02-01")

# store valid dates
valid_dates <- unique(census_ap_3$agg_date)
...
for (date in valid_dates) {
  ...
  month_map = census_ap_3 %>%
    subset(agg_date == date)
  
  month_plot <- ggplot() + 
    geom_sf(
      data = month_map,
      aes(fill = avg_monthly_co), 
    ) + 
    scale_fill_gradient2(low="#789d4a", mid="#eaaa00", high="#A4343A", midpoint=median(month_map$avg_monthly_co)) +
    scale_color_gradient2(low="#789d4a", mid="#eaaa00", high="#A4343A", guide="none") +
    labs(title="Average Daily CO",
